                                                       ...
                                                       std::optional<sch_mcs_index> olla_mcs)
{
  ...
  // Also update ML OLLA algorithm state
  update_olla_state(ml_dl_olla_state, ack_value);
}
...
std::optional<sch_mcs_index> ue_link_adaptation_controller::calculate_dl_mcs(pdsch_mcs_table mcs_table) const
{
  ...
  // We use the raw CQI to estimate SINR, since our AI handles OLLA internally.
  const float raw_cqi = static_cast<float>(get_wideband_cqi());
  if (raw_cqi <= 0.0F) {
    // Special case, where reported CQI==0.
    return std::nullopt;
  }
  
  double measured_sinr_db = cqi_to_sinr_db(raw_cqi);
  double measured_speed_kmph = 30.0; // Stub: Urban mobility
  double channel_ordinal = 1.0;      // Stub: TDL-B
  double carrier_band = 0.0;         // Stub: FR1
  double num_antennas = 2.0;         // Stub: 2x2 MIMO 
  double bler_target_log10 = -1.0;   // eMBB (10% BLER)
  
  // Call Machine Learning Agent
  int prediction = select_mcs(measured_sinr_db,
                              measured_speed_kmph,
                              channel_ordinal,
                              carrier_band,
                              num_antennas,
  ...
  sch_mcs_index mcs{static_cast<uint8_t>(prediction)};
